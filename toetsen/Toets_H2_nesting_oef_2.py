...
for persoon, scores in sport_uitslagen.items():
    gemiddelde_scores[persoon] = sum(scores) / len(scores)
    
hoogste_gemiddelde = max(gemiddelde_scores, key=gemiddelde_scores.get)
laagste_gemiddelde = min(gemiddelde_scores, key=gemiddelde_scores.get)

print(f"Meest: {hoogste_gemiddelde}  ({gemiddelde_scores[hoogste_gemiddelde]:} doelpunten per match)")
print(f"Minst: {laagste_gemiddelde}  ({gemiddelde_scores[laagste_gemiddelde]:} doelpunten per match)")